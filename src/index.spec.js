...
describe('env', () => {
  it('should store variables for an app', async () => {
    ...
    await expect(env.delete({ app: '', key: '' })).rejects.toEqual(new Error('App not specified'));
    await expect(env.delete({ app: 'test', key: '' })).rejects.toEqual(new Error('Key not specified'));
